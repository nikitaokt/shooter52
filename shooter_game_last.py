...
def show_label(text: str, x: int, y: int, font_name: str ='Arial',font_size:int = 40, color:tuple=(255, 255, 255),font_size:int = 40,color:tuple = WHITE)-> None:
    '''
...
class Live:
    '''Жизни для игрока'''
    def __init__(self, x:int,y:int, image_name:str, lives:str)->None:
        '''Аргументы:
    x,y = координаты, где будет выведена надпись
    text = текст надписи
    font_name = название шрифта
...
class GameSprite(sprite.Sprite):
    ...
    def __init__(self, image_name:str, speed:int, x:int, y:int):
        super().__init__()
        '''Аргументы:
        image_name=имя картинки
        speed-скорость
        x,y- координаты от которых начинается отрисовка жизней 
        '''
...
class Enemy(GameSprite):
    ...
    def update(self)->None:
        ...
        если враг долетает вниз игровой системы то умеличивается счетчик пропущеных врагов'''
        self.rect.y += self.speed
        if self.rect.y >= SCREEN_SIZE[1]:
            self.rect.y = 0
            self.rect.x = randint(0, SCREEN_SIZE[0] - SPRITE_SIZE)
            missed_counter.counter += 1
            missed_counter.set_text(24,WHITE)
...
class Heart(GameSprite):
    '''спавн жизни'''
    def update(self)->None:
        '''движение и случайное появление'''
        self.rect.y += self.speed
        if self.rect.y < SCREEN_SIZE[1]:
            self.reset()
        else:
...
class Counter:
    '''расположение текста и счетчик ввиде надписи на экране'''
    ...
    def render_text(self,font_size,text_color):
        f = font.SysFont('Arial',font_size:int)
        self.image = f.render(self.text + str(self.counter),True,text_color:tuple)
        '''рендерит тект в картинку'''
